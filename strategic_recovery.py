...
def get_master_data():
    ...
    # Strategic Vendor List - FULLY POPULATED DATA
    ...
    vendor_data = pd.DataFrame({
        ...
        'Renewal Date': [today + timedelta(days=x) for x in [45, 180, 20, 210, 90]]
    })
    ...
    geo_df = pd.DataFrame({'State': ['NY', 'CA', 'TX', 'FL'], 'Exceptions': [2, 12, 1, 8]})
...
with tab3:
    ...
    with col_gauge:
        fig_gauge = go.Figure(go.Indicator(mode="gauge+number+delta", value=vend_df['YTD Spend ($M)'].sum(), title={'text': "OpEx Budget Utilization ($M)"}, delta={'reference': 500, 'increasing': {'color': "red"}}, gauge={'axis': {'range': [None, 500]}, 'bar': {'color': "#007bff"}, 'threshold': {'line': {'color': "red", 'width': 4}, 'value': 450}}))
    ...
    **Strategic Insight: Why Tier 2 (Legal) carries higher 'Capital Drag'**
    Unlike mass agencies, Tier 2 Legal partners manage accounts in litigation status. Under Basel III 'Loss Given Default' (LGD) models, these require higher capital reserves while in the 12-24 month legal pipeline. This drag is accepted for a **3.4x yield premium** over debt sales.
    """)
    ...
    # REPAIRED: Extraction via .iloc for math
...
with tab5:
    ...
    st.write(f"- **Capital Value:** Simulation indicates +{cet1_bps:.2f} bps CET1 headroom through optimized litigation.")
    st.write("- **Historical Context:** Proactive 2024 optimization curbs risk migration in Branded Cards.")
